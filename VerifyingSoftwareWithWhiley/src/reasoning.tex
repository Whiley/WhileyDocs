...
Recall the definition of the \lstinlinec{decrement()} function from \S\ref{r_functions}.  Since we know \lstinlinec{x > 0} is true before the function call, we know: firstly, that the precondition of \lstinlinec{decrement()} is satisfied; hence, secondly, that the postcondition of \lstinlinec{decrement()} implies \lstinlinec{x >= 0} after the statement.
