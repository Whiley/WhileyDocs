...
Here, the conditional provides more information about the variable \lstinlinec{x}.  On the true branch, we have that \lstinlinec{x >= 0} and this must imply the post-condition \lstinline{r >= 0} where \lstinline{r == x} (which it does).  On the false branch, we have that \lstinline{x < 0} and this must imply the post-condition \lstinline{r >= 0} where \lstinline{r == -x} (which it does).
