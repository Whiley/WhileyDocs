...
The above illustrates a method which accepts two references to variables of type \lstinline{int} (note that these may reference the same variable).  The method simply swaps the contents of the variables to which they refer.

\paragraph{Semantics.}

\paragraph{Notes.}  The dereference operation ``\lstinline{e->f}'' (as in e.g. \lstinline{pX->field}) is a short-hand notation for ``\lstinline{(*e).f}'' and can be used when \lstinline{e} has effective record type (\S\ref{c_types_effective_records}).
