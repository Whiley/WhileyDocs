...
The Whiley programming language is {\em statically typed}, meaning that every expression has a type determined at compile time, such that any evaluation of that expression yields a value of that type.  Whiley's {\em type system} governs how the type of any variable or expression is determined.  Whiley's type system is unusual in that it incorporates {\em union}, {\em intersection} and {\em negation types}, as well as employing {\em flow typing} and {\em structural typing}.
...
int|null id(null|int x):
    return x
\end{lstlisting}
In this function we see two distinct \gls{type_descriptor}s expressed in the program text, namely ``\lstinline{int|null}'' and ``\lstinline{null|int}''.  Type descriptors occur at the source-level and describe \gls{type}s which occur at the abstract (or underlying) level.  In this particular case, we have two distinct type descriptors which describe the {\em same} underlying type --- or, from the perspective of Whiley, they are considered identical.  We will often refer to types as providing the semantics (i.e. meaning) of type descriptors.  

\subsection{Type Semantics}

Although types are abstract entities we can (for the most part) imagine them as describing sets of {\em abstract values}.  For example, \lstinline{int|null} denotes the set of values containing exactly the (infinite) set of integers and \lstinline{null} (i.e. ${\tt\mathbb{Z}\cup\{null\}}$).  This is often referred to as set-theoretic interpretation of types~\cite{AW93,Damm94,CF05,FCB08}.  Under this interpretation, for example, one type {\em subtypes} another if the set of values it denotes is a {\em subset} of the other.  

We specify the meaning of types by formalising a set theoretic interpretation of them over the language of values given in Figure~\ref{c_types_abstract_values}.  To avoid confusion, care is taken in the figure to ensure that abstract values are represented canonically.  For example, ``${\tt 2\;/\;4}$'' is not a valid abstract value; rather ``${\tt 1\;/\;2}$'' is its canonical representation. Likewise, ``${\tt\{2,1\}}$'' is not a valid abstract value, with ``${\tt\{1,2\}}$'' being its canonical representation.   Figure~\ref{c_types_abstract_values} separates abstract values into distinct categories (e.g. integers, rationals, tuples, etc).  These distinctions are significant.  For example, ``${\tt 0}$'' is distinct from ``${0\;/\;0}$''.  Similarly, byte values are not expressed in the digits ${\tt 0}$ and ${\tt 1}$ (as might be expected), but in terms of the characters ${\tt t}$ and ${\tt f}$.  This ensures binary values are distinct from integer values.
...
A value in Whiley may be stored in a variable of some kind (e.g. local variable, parameter, heap location, etc) and is the result of evaluating an expression.  Every value is associated with a unique (i.e. most precise) {\em concrete type}.  For example, the value ${\tt 1}$ is associated with the type \lstinline{int}, whilst the value ${\tt [1,2.2]}$ is \hl{Is abstract?}{associated} with the type \lstinline{[int|real]}.\\

Values are generated by evaluating {\em expressions} given in the program source.  Primitive and compound values are generated by evaluating {\em constructor expressions} given in the program source, as described by the following grammar:

\begin{syntax}
  \verb+ConstructorExpr+ & $::=$ & \\
  & $|$ & \verb+NullExpr+ \\
  & $|$ & \verb+BoolExpr+ \\
  & $|$ & \verb+ByteExpr+ \\
  & $|$ & \verb+IntExpr+ \\
...
The grammar describing the individual constant and constructor expressions are given in the remaining sections of this chapter.
...
&&\\
\verb+BoolExpr+ & $::=$ & \token{true} $|$ \token{false} \\
\end{syntax}
...
\begin{syntax}
 ...
 &&\\
 \verb+ByteExpr+ & $::=$ & \big(\ \token{0}\ $|$\ \token{1}\ \big)$^+$\ \token{b}\\
\end{syntax}
...
\begin{syntax}
  ...
  &&\\
  \verb+IntExpr+ & $::=$ & \big( \token{0}\ $|$\ \ldots\ $|$\ \token{9}\ \big)$^+$ \\
...
\begin{syntax}
  ...
  &&\\
  \verb+RealExpr+ & $::=$ & \big( \token{0}\ $|$\ \ldots\ $|$\ \token{9}\ \big)$^+$\ \token{.}\ \big( \token{0}\ $|$\ \ldots\ $|$\ \token{9}\ \big)$^+$ \\
\end{syntax}
...
\begin{syntax}
  ...
  &&\\
  \verb+TupleExpr+ & $::=$ & \verb+Expr+\ \big(\ \token{,}\
  \verb+Expr+\ \big)$^+$\\
\end{syntax}
