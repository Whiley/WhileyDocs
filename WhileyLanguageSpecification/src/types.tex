...
\label{c_types_value_constructors}

A {\em value} in Whiley may be stored in a variable of some kind (e.g. local variable, parameter, heap location, etc) and is the result of evaluating an expression.  Every value is associated with a unique (i.e. most precise) {\em concrete type}.  For example, the value ${\tt 1}$ is associated with the type \lstinline{int}, whilst the value ${\tt [1,2.2]}$ is \hl{Is abstract?}{associated} with the type \lstinline{[int|real]}.\\

Values are generated by evaluating {\em expressions} given in the program source.  Values can be generated indirectly by performing operations on existing values, or directly by evaluating {\em constructor expressions}, whose top-level grammar is given below:
