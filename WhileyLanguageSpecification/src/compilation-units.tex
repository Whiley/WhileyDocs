...
& $|$ & $($\verb+Modifier+$)^*$ \verb+VariableDecl+\\
...
A {\em variable declaration} declares a variable within a Whiley
\gls{source_file} with an optional initialiser expression.

\begin{syntax}
  \verb+VariableDecl+ & $::=$ & \verb+Type+ \verb+Ident+\ \big[= \verb+Expr+\big]\\
\end{syntax}

\hl{FIXME}{The} given {\em constant expression} is evaluated at {\em compile time}
