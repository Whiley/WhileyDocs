...
\begin{syntax}
  ...
  \big[\ \verb+Type+\ $|$\ \token{(}\ \verb+Type+\ \verb+Ident+\ \token{)}\ \big]\ \big(\ \token{where}\ \verb+Expr+\ \big)$^*$\\
\end{syntax}
...
\begin{syntax}
  \verb+MethodDecl+ & $::=$ & \token{method}\ \verb+Ident+\ \token{(}\ \verb+Parameters+\ \token{)}\ \token{->}\ \token{(}\ \verb+Parameters+\ \token{)}\\
  && \big(\ \token{requires}\ \verb+Expr+\ $|$\ \token{ensures}\ \verb+Expr+\ \big)$^*$\ \token{:}\ \verb+Block+\\
\end{syntax}
