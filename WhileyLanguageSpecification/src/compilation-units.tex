...
\begin{syntax}
  \verb+FunctionDecl+ & $::=$ & \token{function}\ \verb+Ident+\ \token{(}\ \verb+Variables+\ \token{)}\ \token{->}\ \token{(}\ \verb+Variables+\ \token{)}\\
  && \big(\ \token{requires}\ \verb+Expr+\ $|$\ \token{ensures}\ \verb+Expr+\ \big)$^*$\ \token{:}\ \verb+Block+\\
  &&\\
  \verb+Variables+ & $::=$ & \big[ \verb+Variable+ \big( \token{,}\ \verb+Variable+ \big)$^*$\ \big]
\end{syntax}
...
\begin{syntax}
  \verb+MethodDecl+ & $::=$ & \token{method}\ \verb+Ident+\ \token{(}\ \verb+Variables+\ \token{)}\ \token{->}\ \token{(}\ \verb+Variables+\ \token{)}\\
  && \big(\ \token{requires}\ \verb+Expr+\ $|$\ \token{ensures}\ \verb+Expr+\ \big)$^*$\ \token{:}\ \verb+Block+\\
\end{syntax}
