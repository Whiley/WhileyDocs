...
\begin{syntax}
\verb+Keyword+ & $::=$ & \token{any} $|$
\token{assert} $|$ \token{bool} $|$ \token{case} $|$ \token{char} $|$
\token{define} $|$ \token{exists}\\
{\huge\strut} & $|$ & \token{false} $|$ \token{forall} $|$ \token{function} $|$ \token{if} $|$ \token{import} $|$ \token{in} $|$ \token{int} $|$ \token{is}\\
{\huge\strut} & $|$ & \token{no} $|$ \token{null} $|$ \token{package} $|$ \token{private} $|$ \token{protected} $|$ \token{public}\\
{\huge\strut} & $|$ & \token{real} $|$ \token{some} $|$ \token{string}
$|$ \token{then} $|$ \token{true} $|$ \token{void}\\
\end{syntax}
