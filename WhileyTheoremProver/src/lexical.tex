...
\begin{syntax}
\verb+Keyword+ & $::=$ & \token{any} $|$ \token{assume} $|$
\token{assert} $|$ \token{bool} $|$ \token{case} $|$ \token{char} $|$
\token{define}\\
{\huge\strut} & $|$ & \token{ensures} $|$ \token{exists} $|$ \token{false} $|$ \token{forall} $|$ \token{function} $|$ \token{if}\\
{\huge\strut} & $|$ & \token{import} $|$ \token{in} $|$ \token{int} $|$ \token{is} $|$ \token{no} $|$ \token{null} $|$ \token{package} $|$ \token{private}\\
{\huge\strut} & $|$ & \token{protected} $|$ \token{public} $|$ \token{real} $|$ \token{requires} $|$ \token{some} $|$ \token{string}\\
{\huge\strut} & $|$ & \token{then} $|$ \token{throws} $|$ \token{true} $|$ \token{void} $|$ \token{where}\\
...
\begin{syntax}
  \verb+DecimalLiteral+ & $::=$ & \big( \token{0}\ $|$\ \ldots\ $|$\ \token{9}\ \big)$^+$\ \token{.}\ \big( \token{0}\ $|$\ \ldots\ $|$\ \token{9}\ \big)$^+$ \\
\end{syntax}
