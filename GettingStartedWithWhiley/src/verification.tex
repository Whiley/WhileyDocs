...
\begin{lstlisting}
...
// If return is null, there is no index i where items[i] == item
ensures no { i in 0..|items| | items[i] == item }:
    ...
\end{lstlisting}
...
also not a direct translation of the original property, it is a rather
convenient translation which achieves the same thing.