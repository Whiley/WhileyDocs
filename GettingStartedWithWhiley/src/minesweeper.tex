...
The next function we'll implement is that for exposing a square.  This requires that the square to be exposed is not already exposed.  Furthermore, in the case of a blank square, then the expose method is recursively applied to blank squares.  Here is our implementation:

\begin{lstlisting}
function exposeSquare(Board b, int col, int row) => Board:
   Square sq = getSquare(b,col,row)
   // check whether blank square or hidden square
   int rank = determineRank(b,col,row)
   if rank == 0:
      // is blank square so attempt to recursively expose
      b = recursivelyExposeFrom(b,col,row)
   else if sq is HiddenSquare:
      // is hidden square so expose
...
function recursivelyExposeFrom(Board b, int col, int row) => Board:
   ...
   Square sq = getSquare(b,col,row)
   int rank = determineRank(b,col,row)
   if sq is HiddenSquare && rank == 0:       
      // yes, so expose square
      sq = ExposedSquare(0,0)
      b = setSquare(b,col,row,sq)
      // now expose neighbours
      b = recursivelyExposeFrom(b,col-1,row-1)
      b = recursivelyExposeFrom(b,col,row-1)
      b = recursivelyExposeFrom(b,col+1,row-1)
      b = recursivelyExposeFrom(b,col-1,row)
      b = recursivelyExposeFrom(b,col+1,row)
      b = recursivelyExposeFrom(b,col-1,row+1)
      b = recursivelyExposeFrom(b,col,row+1)
      b = recursivelyExposeFrom(b,col+1,row+1)
